...
const create = catchError(async(req, res) => {
    const {email,password,firtName, lastName, country, image, frontBaseUrl} = req.body;
    ...
    await sendMail({
        ...
        html: ` <h1>Hello ${firtName} ${lastName}</h1>
        <p>Verify your accout clicking this link </p>
...
const verifyCode = catchError(async (req, res) => {
    ...
    const user = await User.update({ isVerified: true }, { where: { id:codeFound.userId}, returning: true});
...
const link = `${frontBaseUrl}/reset_password/${code}`
    await sendMail({
        ...
        html: ` <h1>Hello</h1>
        <p>Reset Password  clicking this link </p>
        <a href="${link}" target="_blank">${link}</a>
        <h3>Thanks you</h3>
        `
    });
...
const resetPassword  = catchError(async (req, res)=>{
    ...
    if (!codeFound) return res.status(401).json({ message: "Invalid code"});
    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await User.update({ password: hashedPassword }, { where: { id:codeFound.userId}, returning: true});
